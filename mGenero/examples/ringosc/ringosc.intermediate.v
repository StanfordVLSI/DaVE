...
$${
digital_modes = [get_lm_equation_modes('test2', 'actl_ctr')]
digital_cases = [digital_modes[0][0].keys()]
variable_map = {}
}$$

$$[if not mode_exists('test2')]
  actl_ctr = $$get_lm_equation('test2', 'actl_ctr');
  freq_max = $$get_lm_equation('test2', 'freqout_max');
  vi_max = $$get_lm_equation('test2', 'actl_max');
  freq_ctr = $$get_lm_equation('test2', 'freqout_ctr');
  kvco = $$get_lm_equation('test2', 'kvco');
  freq_min = $$get_lm_equation('test2', 'freqout_min');
  vi_min = $$get_lm_equation('test2', 'actl_min');
$$[else]
...
$$[for m in digital_modes[0]]
  {$$(','.join(["%d'b%s" % (Pin.vectorsize(d), dec2bin('%d'%m[d], Pin.vectorsize(d))) for d in digital_cases[0]]))}: begin
    actl_ctr = $$get_lm_equation('test2', 'actl_ctr', m);
    freq_max = $$get_lm_equation('test2', 'freqout_max', m);
    vi_max = $$get_lm_equation('test2', 'actl_max', m);
    freq_ctr = $$get_lm_equation('test2', 'freqout_ctr', m);
    kvco = $$get_lm_equation('test2', 'kvco', m);
    freq_min = $$get_lm_equation('test2', 'freqout_min', m);
    vi_min = $$get_lm_equation('test2', 'actl_min', m);
  end
$$[end for]
  default: begin
    actl_ctr = $$get_lm_equation('test2', 'actl_ctr', digital_modes[0][0]);
    freq_max = $$get_lm_equation('test2', 'freqout_max', digital_modes[0][0]);
    vi_max = $$get_lm_equation('test2', 'actl_max', digital_modes[0][0]);
    freq_ctr = $$get_lm_equation('test2', 'freqout_ctr', digital_modes[0][0]);
    kvco = $$get_lm_equation('test2', 'kvco', digital_modes[0][0]);
    freq_min = $$get_lm_equation('test2', 'freqout_min', digital_modes[0][0]);
    vi_min = $$get_lm_equation('test2', 'actl_min', digital_modes[0][0]);
  end
  endcase
$$[end if]
