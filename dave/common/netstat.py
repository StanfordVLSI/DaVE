...
def netstat():
  ...
  for line in content:
    ...
    nline = [tcp_id, uid, l_host+':'+l_port, r_host+':'+r_port, state]
    result.append(nline)
  return result
...
def get_foreign_ip_address(port=5000, status='ESTABLISHED'):
  return [c[3][:c[3].rfind(':'+str(port))] for c in netstat() if c[3].endswith(str(port)) and c[4]==status]

def get_established_foreign_ip_address(port=5000):
  return get_foreign_ip_address(port)
